...
A~\code{CloneResult} is a more compact and convenient representation of the \code{CloneClass}es. Each has the text it represents as its \textit{name} and a \code{List} of all occurrences associated, whereas a occurrence is a \code{Pair} representing the \textit{originID} (file identifier) and the \textit{line~number}.\\
